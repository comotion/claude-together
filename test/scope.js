...
// directory, shared only via an explicit CLAUDE_TOGETHER_DIR override.
...
function tmpdir (label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `ct-scope-${label}-`))
}

const savedDir = process.env.CLAUDE_TOGETHER_DIR
...
console.log('3. CLAUDE_TOGETHER_DIR overrides scoping with one exact directory…')
...
if (savedDir === undefined) delete process.env.CLAUDE_TOGETHER_DIR
else process.env.CLAUDE_TOGETHER_DIR = savedDir
